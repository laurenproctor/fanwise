...
import { newCreator, signOut, uniqueEmail } from "./support"
 ...
 *
 * A malformed address is refused by the server action before the auth server
 * is asked anything; that is `tests/unit/password-recovery.test.ts`, which
 * calls the action itself.
 *
...
test("the answer is identical for a registered and an unregistered address", async ({ page }) => {
  const { email } = await newCreator(page, "recovery")
  await signOut(page)

  // Reached the way a signed-out creator reaches it: from sign in.
...
test("a spent or forged link explains itself instead of failing silently", async ({ page }) => {
  ...
  // And the reset page itself, opened after its session is gone, says so rather
  // than bouncing to sign in. Without /reset-password in the proxy's public
  // paths this lands on /sign-in, which tells a person whose link expired
  // nothing about why.
