 ...
 * channels that can take the product, says what it skipped and why, and
 * refuses to send the same thing twice.
 *
 ...
 * that makes a second attempt create nothing.
 */

test("one click publishes what it can and names what it skipped", async ({ page }) => {
  const { slug } = await signUpAndCreateWorkspace(page, "j5e", "Everywhere Studio")

  // One channel that can publish and one that structurally cannot. The second
  // is the case the run has to report rather than quietly omit.
...
test("a second run sends nothing again, and says why", async ({ page }) => {
  const { slug } = await signUpAndCreateWorkspace(page, "j5e2", "Repeat Studio")
  await connect(page, slug, "Mock Storefront")
  ...
  // The channel holds the product and there is nothing new to send, so the run
  // has nothing to do and the button does not pretend otherwise.
  ...
  // And exactly one listing, one link: a second run created nothing.
  await expect(page.getByRole("link", { name: /View on Mock Storefront/ })).toHaveCount(1)
})

test("the activity log records what the run did", async ({ page }) => {
  const { slug } = await signUpAndCreateWorkspace(page, "j5ea", "Activity Studio")
  await connect(page, slug, "Mock Storefront")
  ...
  await page
    .getByRole("region", { name: "Publish everywhere" })
    .getByRole("button", { name: "Publish everywhere" })
    .click()
  await expect(page.getByText("Live", { exact: true })).toBeVisible({ timeout: 30_000 })

  // The run, and the job that settled inside it. Written as they happened, and
  // still here after a reload because the rows cannot be edited or removed.
  await page.reload()
...
test("offers nothing to press when no connected channel can publish", async ({ page }) => {
  const { slug } = await signUpAndCreateWorkspace(page, "j5eo", "Assisted Only Studio")
  await connect(page, slug, "Mock Marketplace")

  await page.goto(`/${slug}/new`)
  await page.getByLabel("Product name").fill("Assisted Only")
  await page.getByRole("button", { name: "Create product" }).click()
  await waitForProductPage(page, slug, "Assisted Only")
  await page.getByRole("button", { name: "Build listing" }).click()

  // Absent, not disabled. A greyed-out Publish Everywhere promises an action
  // that will never work on a channel with no publish method, which is the
  // rule the listing cards already follow. Journey 3 asserts the same thing
  // from the other side, over every button on the page.
  ...
  await connect(page, slug, "Mock Storefront")
  await page.goto(`/${slug}/assisted-only`)
