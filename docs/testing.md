...
**B11 adds no journey.** The companion window, planned on 12 September 2026 and built on
13 September 2026 in `docs/companion-window.md`, is journey 7 run in a second window, not a
new path from empty workspace to live listing. `tests/e2e/companion-window.spec.ts` opens the
real window in Chromium, copies from inside it and brings it back;
`tests/unit/handoff.test.ts` and `tests/unit/companion.test.ts` hold the steps, the no-API
