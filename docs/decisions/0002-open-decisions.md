...
| Shopify file delivery | A Fanwise download link in a product metafield, printed by the shop's order confirmation email after a one-time snippet; readiness blocks until the creator confirms it. Decided 15 September 2026 at the founder's request, replacing ADR 0001's assisted step | `docs/decisions/0013`, `docs/channels/shopify.md` §6 |
| WooCommerce file delivery | A Fanwise download address per file, sealed token, re-checked per request, redirect to a five-minute link; replaceable from the listing card. Decided 13 September 2026 at the founder's request, replacing B8's assisted file step | `docs/decisions/0012`, `docs/channels/woocommerce.md` §6 |
